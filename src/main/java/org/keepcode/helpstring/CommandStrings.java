package org.keepcode.helpstring;

public class CommandStrings {

  public static final String SVR_REBOOT_DEV = "svr_reboot_dev %d %s";
  public static final String GET_GSM_NUM = "get_gsm_num %d %s";
  public static final String SVR_REBOOT_MODULE = "svr_reboot_module %d %s";
  public static final String SEND_USSD = "USSD %d %s %s"; //todo нормально назвать и сгруппировать по назначения set/get gsm
  public static final String SET_GSM_NUM = "set_gsm_num %d %s %s";
  public static final String MSG = "MSG %d %d %s\n";
  public static final String PASSWORD = "PASSWORD %d %s\n";
  public static final String SEND = "SEND %d %d %s\n";
  public static final String DONE = "DONE %d\n";
  public static final String REG_STATUS_MSG = "reg:%s;status:%d;";
  public static final String RECEIVE_OK_MSG = "RECEIVE %s OK\n";
  public static final String STATE_OK_MSG = "STATE %s OK\n";
}
